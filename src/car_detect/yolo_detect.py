        ...
        if boxes is not None:
            ...
            for box_idx, box in enumerate(boxes):
                ...
                try:
                    ...
                    if not hasattr(simple_yolo_ocr, 'easyocr_reader'):
                        print("🔄 Инициализация EasyOCR для проверки...")
                        simple_yolo_ocr.easyocr_reader = easyocr.Reader(['ru', 'en'], gpu=False)
                    
                    reader = simple_yolo_ocr.easyocr_reader
                    results = reader.readtext(plate_roi, detail=0, paragraph=True)

                    if results:
                        # Объединяем все найденные тексты
                        detected_text = ' '.join(results).strip()
                        
                        # Дополнительная проверка что это похоже на номер
                        # (минимум 2 символа, содержащие буквы и/или цифры)
                        if len(detected_text) >= 2 and any(c.isalnum() for c in detected_text):
                            # Очищаем текст для вывода
                            import re
                            clean_text = re.sub(r'[^A-ZА-Я0-9\s]', '', detected_text.upper()).strip()
                            
                            print(f"   • ✅ EasyOCR подтвердил текст: '{clean_text}'")
                            
                            # Сохраняем в сером формате
                            gray = cv2.cvtColor(plate_roi, cv2.COLOR_BGR2GRAY)
                            cv2.imwrite(plate_filename, gray)
                            print(f"   • 📁 Сохранен как: {plate_filename}")
                    ...
                    print(f"   • ⚠️ Ошибка проверки EasyOCR: {str(e)[:50]}...")
                    
                    # В случае ошибки сохраняем для отладки
                    gray = cv2.cvtColor(plate_roi, cv2.COLOR_BGR2GRAY)
                    cv2.imwrite(plate_filename, gray)
                    print(f"   • 📁 Сохранен (с ошибкой): {plate_filename}")
